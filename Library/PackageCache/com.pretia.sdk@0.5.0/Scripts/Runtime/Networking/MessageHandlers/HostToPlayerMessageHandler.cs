...
{
    public class HostToPlayerMessageHandler
    {
        protected ISerializer _serializer;
        private ReliableGameClient _gameClient;
        private TickManager _tickManager;

        private OpType OP_TYPE => OpType.HostToPlayer;
        ...
        public HostToPlayerMessageHandler(ISerializer serializer, ReliableGameClient gameClient, TickManager tickManager)
        {
        ...
        public void Enqueue<T>(T data)
        {
            ...
            _gameClient.EnqueueHostToPlayerMessage(body, msgType, _tickManager.Tick);
        }

        public virtual void Send<T>(T data, Player target)
        {
            byte[] body = _serializer.Serialize(data);
            ushort msgType = TypeResolver.Get<T>();

            _gameClient.SendHostToPlayerMessage(body, msgType, target.UserNumber, _tickManager.Tick);
        }

        public virtual void SendQueue(Player target)
        {
            _gameClient.SendHostToPlayerMessage(target.UserNumber, _tickManager.Tick);
        }
