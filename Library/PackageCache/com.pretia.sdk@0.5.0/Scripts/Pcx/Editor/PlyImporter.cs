...
namespace Pcx
{
    ...
    public class PlyImporter : UnityEditor.AssetImporters.ScriptedImporter
    {
...
#if UNITY_EDITOR_OSX
                var renderer = gameObject.AddComponent<MultiModePointCloudRenderer>();
        ...
        {
            // Via package manager
            var path_upm = "Packages/com.pretia.sdk/Scripts/Pcx/Editor/Default Point.mat";
            // Via project asset database
            var path_prj = "Assets/PretiaSDK/Scripts/Pcx/Editor/Default Point.mat";
        ...
        protected virtual Mesh ImportAsMesh(string path)
        {
            try
            {
        ...
        protected virtual PointCloudData ImportAsPointCloudData(string path)
        {
            try
            {
                var stream = File.Open(path, FileMode.Open, FileAccess.Read, FileShare.Read);
        ...
        protected virtual BakedPointCloud ImportAsBakedPointCloud(string path)
        {
            try
            {
                var stream = File.Open(path, FileMode.Open, FileAccess.Read, FileShare.Read);
